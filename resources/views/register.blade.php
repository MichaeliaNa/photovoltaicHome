    ...
    $('.bk_phone_code_send').click(function(event) {
      ...
      var interval = window.setInterval(function() {
        ...
        if(num == 0) {
          ...
          $('.bk_phone_code_send').html('重新发送');//提示重新发送
        }
      }, 1000);//1s
      ...
      dataType: 'json',
      cache: false,
      data: {phone: phone, _token: "{{csrf_token()}}"},//阶层对象
        success: function(data) {
          if(data == null) {
            ...
            setTimeout(function() {$('.bk_toptips').hide();}, 2000);//2s后隐藏
            return;
          }
          if(data.status != 0) {
            $('.bk_toptips').show();
    ...
    function onRegisterClick() {
      ...
      $('input:radio[name=register_type]').each(function(index, el) {
        if($(this).attr('checked') == 'checked') {
          ...
          $.ajax({
            type: 'POST',//_token->CRSF Protection
            url: '/service/register',
            dataType: 'json',
            cache: false,
            data: {phone: phone, email: email, password: password, confirm: confirm,
            phone_code: phone_code, validate_code: validate_code, _token: "{{ csrf_token() }}" },//""->string
            success: function(data) {
              if(data == null) {
                $('.bk_toptips').show();
                $('.bk_toptips span').html('服务端错误');
